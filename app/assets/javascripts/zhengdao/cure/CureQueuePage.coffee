    ...
    <div className='cure-queue-page with-tabs'>
      <div className='tabs'>
        <div className={klass1}>
          <a className='label' href={@props.data.default_queue_url}>挂号({@props.data.wait_queue_count})</a>
        </div>
        <div className={klass2}>
          <a className='label' href={@props.data.pe_queue_url}>医师推送({@props.data.send_queue_count})</a>
        </div>
      </div>
